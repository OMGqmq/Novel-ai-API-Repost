...
function switchAdminTab(tab) {
    ...
    const inactiveClass = 'flex-1 text-center py-2 text-[11px] md:text-xs font-semibold rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 transition-all';
...
function switchToolboxTab(tabId) {
...
async function toggleScrambleHistoryList() {
...
function selectScrambleHistoryImage(imageSrc) {
    if (!imageSrc) return;
    loadScrambleCanvas(imageSrc);
    if (window.showToast) window.showToast('成功导入生图历史图片', 'success');
}

function handleScrambleFileUpload(event) {
    const file = event.target.files[0];
    if (!file) return;
...
function loadScrambleCanvas(imgSrc) {
    if (!imgSrc) return;
...
function setScrambleMode(isDecryptMode) {
...
function onScrambleAlgorithmChange() {
...
function toggleScramblePasswordInput() {
...
function executeScrambleProcess() {
...
function downloadScrambleResult() {
    const canvas = document.getElementById('scrambleCanvas');
    if (!canvas) return;
